...
| R-20 | Load CompLib in full | **Vibe** | **Load done, export outstanding** — draft PR #26. The full walk ran: 86,054 compositions across all 3,443 pages of `/composition/search`, matching the API's `count` to the record, with the 3,443 raw pages cached in `complib-cache/`. Nothing is lost — but both that cache and the database it loaded are gitignored, so **a fresh clone still builds zero composition tables**: `schema/006_init_complib.sql` is never applied by `build_local_db.py`, and no CSV is committed. Every other source lands as CSVs replayed at build time (`data/bellboard/`, 52 files, globbed by `build_local_db.py:121`), which is what lets `check_csv_agreement` catch a stale replica. **Remaining work needs no API access** — the cache holds everything, so the export is a local transform: apply the schema, write `data/complib/*.csv`, add a row-count check. Weigh against R-34: `data/bellboard/` is already 149 MB. If that size is unwelcome, the honest alternative is to leave the data out and *not* call this Done |
...
| R-35 | A removal / anonymisation request route | **Claude Code** | Mine, next. Names are published as a deliberate decision (`docs/privacy_and_licence_audit.md`) because BellBoard is public — but a ringer who objects should have somewhere to go. GitHub Pages is static and cannot accept a form post, so the options are a `mailto:` link (private, no third party, no external request — fits this site's constraints), a public GitHub issue (wrong: a removal request would itself be public and permanent), or a hosted form service (adds an external dependency to a site whose whole invariant is not having one). **A link is the easy half.** Honouring a request needs a committed suppression list that every builder consults, or the next `rebuild_all.py` silently reinstates the person — and the request cannot be honoured upstream at all, since BellBoard is the source and stays as it is. Say that on the page rather than implying more than can be delivered |
