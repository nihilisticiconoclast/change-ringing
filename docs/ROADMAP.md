...
| 31 | The rest of the page CSS is still copied eleven times | **Gemini** | **Next for Gemini after item 25.** Item 30 unified the nav; the palette, `body`, `.wrap`, `.eyebrow`, `header`, `h1`, `.figures` and `section` rules are still pasted into every template and builder, and they have already drifted — `.wrap` is 1200px on most pages, 1180px on `ringers.html` and 1120px on `occasions.html`. Same cure: one definition in `site_chrome.py`, a check in `verify_chrome.py`, and delete the copies. Do NOT hand-edit `docs/*.html` |
| 32 | `invention.html` is the only page that phones out | **Gemini** | **Next for Gemini.** The composition visualiser loads `vis-network` from `unpkg.com`. Every other page on this site is a single self-contained file with no external requests, which is stated in the README and is the reason the site works offline, in an archive, and behind a proxy. If unpkg is slow or blocked the page is dead. Either vendor the library into the page or draw the graph with the same inline-SVG approach the other twelve use |
