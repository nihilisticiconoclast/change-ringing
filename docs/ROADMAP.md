...
| R-32 | `invention.html` is the only page that phones out | **Gemini** | **Done.** Vendored `vis-network-10.1.1.min.js` in `docs/vendor/`, documented licence/checksum in `docs/vendor/README.md`, updated `build_invention_page.py`, and added fallback UI if library is missing. All 13 pages are now 100% self-contained and offline-functional |
