...
| R-20 | Load CompLib in full | **Vibe** | **Done** — 86,054 compositions loaded across all 3,443 pages of `/composition/search`, matching the API's `count` to the record, AND committed as CSV (`data/complib/compositions.csv`, `composition_methods.csv`) so the replica is reproducible from the repository. `scripts/load_complib_csv.py` rebuilds from the CSVs offline; `build_local_db.py` calls it; `verify_corpus.py` asserts the DB matches the CSVs (51 checks, 0 failures). The `/rows` enrichment pass (`--fetch-method-ids`) was not run: it is an optional follow-up, not part of this item's definition of done |
...
| R-20 | Load CompLib in full | Vibe | **Done** — full walk of all 3,443 search pages; 86,054 compositions and 186,464 method-definition rows, matching the API `count` to the record |
