...
| R-36 | **JOIN** — bridge CompLib to the performance record by composer | **Claude Code** | Next for me. `performances.composition` is **0% populated**, so a performance can never be tied to a specific CompLib composition — that route is closed and should not be attempted. What does work is the composer: `performances.composer` is free text on 71,959 records, and **92.0% of those (59,452 of 64,612 parseable) name a composer CompLib also holds**, on 1,080 shared first-initial-plus-surname keys. Build that bridge as an adjudicated candidate CSV with a measured accuracy, not a silent join |
...
| R-40 | **VISUALISATION** — what a composition actually asks of a band | **Gemini** | Next for Gemini. `compositions.calling` is populated on **99.6%** of 86,054 rows — the literal call sequence — and nothing has ever read it. `partheads` on 41.3%, `coursehead_masks` on most. Draw the shape of a composition: call density, part structure, how much of the work falls where. This is structural data with no analysis at all behind it |
| R-41 | **JOIN** — normalise composer names properly | **Gemini** | After R-40. My R-36 bridge uses first-initial-plus-surname, which collapses 2,542 CompLib composers to 1,969 keys and 2,737 performance composers to the same shape. That is deliberately crude and its error rate is **unmeasured**. Do it properly with an adjudicated sample, the way `docs/method_location_resolution.md` did — and report where it is wrong |
| R-42 | **TEST** — a golden-file test for the page builders | **Gemini** | The open half of R-26. 36 tests cover `notation.py`, `sqlfile.py`, `site_chrome.py` and the oracles, but **a builder that silently changes a page's output still passes everything** — which is the failure mode that has bitten this project most often. Snapshot each page's structure (not its bytes: figures change with the data) and fail when it moves unexpectedly |
| R-43 | **FIX** — `civic` precision is 38.8% | **Gemini** | The open half of R-3. The occasion classifier measures 75.5% overall, but `civic` precision is 38.8% because the royal-death patterns swallow memorial and funeral records. `docs/footnote_occasion_accuracy.md` recommends not publishing `civic` and `practice` counts; that recommendation is still live. Either fix the patterns and re-measure against the committed 400-row oracle, or suppress those two classes on the page |
...
| R-45 | **FIX** — one database access path (was R-28) | **Vibe** | After R-44. `classify_footnote_occasions.py` and `fetch_and_export_bellboard.py` use stdlib `sqlite3` while ingestion uses `db.py`/libsql, and sqlite3 accepts SQL that libSQL rejects — so a script tested one way is not tested for the other. Two of the three production-only bugs in `docs/LESSONS.md` were exactly this. `db.py`'s docstring explains why |
