...
| R-26 | A real test suite | **Gemini** | **Done.** 36 automated unit and regression tests in `tests/` (`test_notation.py`, `test_sqlfile.py`, `test_site_chrome.py`, `test_oracles.py`, `test_docs.py`), configured in `pytest.ini`, asserted across multi-stage method benchmarks and oracle datasets, and integrated into `.github/workflows/pr-checks.yml` |
