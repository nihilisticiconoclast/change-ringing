...
def apply_chrome(html, dark=False):
    """Expand the NAV and FOOTER markers and apply the base and chrome CSS.

    Raises unless both markers are present exactly once. Strict on purpose: a
    page that silently builds without a footer is how the inconsistency happened
    the first time.

    BASE_CSS is PREPENDED and CHROME_CSS APPENDED, which is not arbitrary. A page
    must be able to add to the base -- its own scale colours, its own charts --
    so the base goes first and loses ties. Nothing may override the nav, so the
    chrome goes last and wins them. verify_chrome.py enforces both directions.
    """
