...
<body>
    ...
    <script>
        ...
        // Add Astrolabe / Celestial Spheres for visual reference (thicker & more opaque for better visibility)
        const scene = Graph.scene();
        const createRing = (radius, color, axis) => {
            const geometry = new THREE.TorusGeometry(radius, 4, 12, 100);
            const material = new THREE.MeshBasicMaterial({ color: color, transparent: true, opacity: 0.25 });
