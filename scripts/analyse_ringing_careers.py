...
def build_careers(conn, ids):
    ...
    unmapped = 0
    for name, date, bell, conductor, perf_id, changes in conn.execute(
        "SELECT TRIM(r.name), p.perf_date, r.bell, r.conductor, r.perf_id, p.changes "
        ...
        if b < 1 or b > n:        # a bell outside the ring is a parse artefact
            continue
        # A peal is >= 5000 changes -- the definition queries/findings/
        # peal_and_quarter_populations.sql already uses. Without it, "conducted
        # a peal" counts every conducted quarter, and the corpus is mostly
        # quarters: 72.5% of the cohort have conducted SOMETHING, but only
        # 20.0% have conducted a peal, and the median wait goes from 11
        # appearances to 37. Two different claims, one of which was published.
        try:
            is_peal = int(changes) >= 5000 if changes not in (None, "") else False
        except (TypeError, ValueError):
            is_peal = False
        careers[cid].append((date, b, n, conductor, is_peal))
...
def q1_apprenticeship(careers, cohort_ids, peals_only=False):
    """Appearances before a first conducted performance (or first conducted peal).

    The unit of the WAIT is always an appearance -- every tower appearance
    counts toward it. What `peals_only` changes is the EVENT being waited for.
    """
    first_conduct_at = []
    ever_conducted = 0
    for cid in cohort_ids:
        ...
        for i, app in enumerate(ordered):
            if app[3] == 1 and (app[4] or not peals_only):   # conductor flag
...
def main():
    ...
    # ---- Q1 ----
    print("1. HOW LONG IS THE APPRENTICESHIP? appearances before a first conducted performance")
    for label, peals_only in (("any performance", False), ("a PEAL (>=5000 changes)", True)):
        ever, firsts = q1_apprenticeship(careers, cohort_ids, peals_only=peals_only)
        fs = sorted(firsts)
        print(f"   conducting {label}:")
        print(f"     {ever:,} of {len(cohort_ids):,} cohort ringers "
              f"({100*ever/len(cohort_ids):.1f}%) ever conduct one in this corpus")
        print(f"     among those who do: median {statistics.median(firsts):.0f}, "
              f"mean {statistics.mean(firsts):.1f}, p25 {percentile(fs,25)}, "
              f"p75 {percentile(fs,75)} appearances first")
    print("   Both are reported because the corpus is mostly quarters, and the two answer\n"
          "   different questions. Conducting SOMETHING is common and happens early;\n"
          "   conducting a PEAL is a minority activity reached much later. An earlier draft\n"
          "   measured the first and called it the second.")
    print("   Skewed either way: the mean is pulled up by a long tail of late conductors.\n")
