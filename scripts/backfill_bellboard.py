...
# A window fetched to at least this fraction of its expected count is
# checkpointed. Pulled from bellboard_common.WINDOW_TOLERANCE by default so
# the two never drift.
...
def is_short(fetched, expected):
    ...
    if expected is None:
        return fetched > 0
    threshold = expected * (1 - WINDOW_TOLERANCE)
...
def store_performances(conn, perf_elements):
    ...
    Returns:
        tuple: (rows_inserted, duplicates_seen, ringers, footnotes, flags)
        where duplicates_seen counts perf_ids that appeared more than once
        within this batch -- a non-zero value means export.php returned the
        same record on more than one page, which is the size-signal
        investigation's smoking gun (see
    ...
    return len(perf_rows), duplicates_seen, len(ringers), len(footnotes), len(flags)
...
def process_window(conn, date_from, date_to, delay, max_pages=0,
    ...
    while True:
        ...
        if not is_short(rows, expected):
            result["complete"] = True
            print(
                f"  Window complete: {rows} fetched vs {expected} expected "
                f"(within {WINDOW_TOLERANCE:.0%} tolerance)"
            )
            return result
...
def main() -> int:
    ...
    if not args.skip_count_gate and corpus_expected is not None:
        ...
        if is_short(db_in_range, corpus_expected):
            ...
            print(f"  OK: within {WINDOW_TOLERANCE:.0%} of the corpus count.")
